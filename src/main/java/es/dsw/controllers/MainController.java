...
public class MainController {
	...
	public String home(Authentication authentication, Model objModel,
			@RequestParam(name = "modificar", required = false, defaultValue = "1") String modificar) {
	...
	public String tareasView(Model objModel, Authentication authentication) {
		...
		ArrayList<Usuario> objTablaOperario = objUsuarioDao.getAllEmployees();
		...
		objModel.addAttribute("Usuarios", objTablaOperario);
		objModel.addAttribute("Roles", objTablaRol);
	...
	public String nuevoUsuario(@RequestParam("nombre") String nombre, @RequestParam("apellidos") String apellidos,
			@RequestParam("direccion") String direccion, @RequestParam("telefono") String telefono,
			@RequestParam("mail") String mail, @RequestParam("nif") String nif,
			@RequestParam("password") String password, @RequestParam("passwordrep") String passwordrep,
			@RequestParam(name = "rol", defaultValue = "-1") int rol, Model objModel) {
		...
		if (nombre != "" /* && matcher.find() */ && password != "" && passwordrep != "" && password.equals(passwordrep)
				&& mail != "" && rol != -1 && password.length() >= 6) {
			...
			/*
			 * if(matcher.find()==false) { mensaje = mensaje +
			 * "El correo introducido no es correcto. "; error = true; }
			 */
	...
	private String updateUsuario(@RequestParam("id") String id, @RequestParam("nombre") String nombre,
			@RequestParam("apellidos") String apellidos, @RequestParam("direccion") String direccion,
			@RequestParam("tlf") String tlf, @RequestParam("mail") String mail, @RequestParam("nif") String nif,
			@RequestParam("password") String password, @RequestParam("rol") String rol, Model objModel) {
		...
		if (nombre != "" /* && password.length() >= 6 && matcher.find()== true */ && password != "" && mail != ""
				&& rol != "" /*
								 * ||(rol == "operario" || rol == "capataz" || rol == "cliente" || rol ==
								 * "administrador")
								 */) {
			...
			/*
			 * if(rol != "operario" || rol != "capataz" || rol != "cliente" || rol !=
			 * "administrador") { mensaje = mensaje +
			 * "Los roles deben ser operario, capataz, cliente o administrador"; error =
			 * true; }
			 */
			/*
			 * if(matcher.find()==false) { mensaje = mensaje +
			 ...
			 */
			/*
			 * if(password.length()<6) { mensaje = mensaje +
			 * "El password debe tener al menos 6 caracteres"; error = true; }
			 */
		}
	...
	public String nuevaTarea(Authentication authentication, Model objModel,
			@RequestParam(name = "explotacion", defaultValue = "-1") int explotacion,
			@RequestParam(name = "operario", defaultValue = "-1") int operario, @RequestParam("tipo") String tipo) {
	...
	private String updateTarea(@RequestParam("id") String id, @RequestParam("usuarioCrea") String usuarioCrea,
			@RequestParam("explotacion") String explotacion, @RequestParam("operario") String operario,
			@RequestParam("status") String status, @RequestParam("tipo") String tipo, Model objModel) {
