...
public class UsuariosDao {
	...
	public ArrayList<Usuario> getAll() {
		...
		try {
			...
			if (!objConnection.isError()) {
				...
				this.flagError = true;
				this.msgError = "Error en getOnce. El objeto MySqlConnection informa error al abrir conexión. +Info: "
						+ objConnection.msgError();
			}
	...
	public ArrayList<Usuario> getAllEmployees() {
		MySqlConnection objConnection = new MySqlConnection();
		ArrayList<Usuario> objTablaUsuario = new ArrayList<Usuario>();
		...
		try {
			objConnection.open();
			if (!objConnection.isError()) {
				ResultSet Result = objConnection.executeSelect(
						"SELECT * FROM usuarios u INNER JOIN usuarios_roles ur ON u.ID = ur.ID_USUARIO AND ur.ID_ROL != 4;");
				while (Result.next()) {
					Usuario objUsuario = new Usuario();
	...
	public Usuario getUserById(int id) {
		...
		try {
			...
			if (!objConnection.isError()) {
				...
				this.flagError = true;
				this.msgError = "Error en getById. El objeto clsConectionMysql informa error al abrir conexión. +Info: "
						+ objConnection.msgError();
			}
		} catch (Exception ex) {
			this.flagError = true;
	...
	public void setUsuario(Usuario objUsuario) {
		...
		try {
			objConnection.open();
			if (!objConnection.isError()) {
				String sql = "INSERT INTO usuarios" + "(NOMBRE," + "APELLIDOS," + "DIRECCION," + "TLF," + "EMAIL,"
						+ "NIF," + "password)" + "VALUES " + "('" + objUsuario.getNombre() + "'," + " '"
						+ objUsuario.getApellidos() + "'," + " '" + objUsuario.getDireccion() + "'," + " '"
						+ objUsuario.getTlf() + "'," + " '" + objUsuario.getMail() + "'," + " '" + objUsuario.getNif()
	...
	public void deleteById(int idUsuario) {
		...
		try {
			...
			if (!objConnection.isError()) {
				...
				this.flagError = true;
				this.msgError = "Error en deleteById. El objeto clsConectionMySql informa error al abrir conexión. +Info: "
						+ objConnection.msgError();
			}
	...
	public void updateUser(Usuario objUsuario) {
		...
		try {
			objConnection.open();
			if (!objConnection.isError()) {
				String sql = "UPDATE usuarios set " + "NOMBRE = '" + objUsuario.getNombre() + "', " + "APELLIDOS = '"
						+ objUsuario.getApellidos() + "', " + "DIRECCION = '" + objUsuario.getDireccion() + "', "
						+ "TLF = '" + objUsuario.getTlf() + "', " + "EMAIL = '" + objUsuario.getMail() + "', "
						+ "NIF = '" + objUsuario.getNif() + "', " + "password = '" + objUsuario.getPassword() + "' "
						+ "WHERE ID = " + objUsuario.getId();
				objConnection.executeUpdateOrDelete(sql);
			} else {
				this.flagError = true;
				this.msgError = "Error en updateUser. El objeto clsConectionMySql informa error al abrir conexión. +Info: "
						+ objConnection.msgError();
			}
		} catch (Exception ex) {
			this.flagError = true;
